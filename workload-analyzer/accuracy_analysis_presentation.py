...
def produce_bar_plot_wt_err_bars(mid_q_dict, upper_q_dict, p99_q_dict, plot_title, x_label, y_label, ylim_top,
    ...
    ax.set_title(plot_title, x=0.12, y=1.0, pad=-16)
...
def plot_hist_dataset(data_df, col_of_interest, x_label, y_label, plot_title, plot_file_name):
    ...
    elif plot_title == 'Distribution used for adaptability':
        sns.histplot(data=data, stat='probability', ax=ax, color='k', binwidth=1)
    else:
    ...
    print(bins)
    padding_size = 20
    plt.xlabel(x_label, labelpad=padding_size)
    plt.ylabel(y_label, labelpad=padding_size)
    plt.title(plot_title, pad=padding_size)
    # fig.tight_layout()
    ax.autoscale(enable=True)
    if plot_title == 'NYT':
        ax.set_xlim(0, 100)
...
if __name__ == '__main__':
    ...
    # ds_to_plot = ['adaptability']
    ds_to_plot = ['pareto', 'uniform', 'power', 'nyt']
    ...
    if data_set_analysis:
        ...
        if 'adaptability' in ds_to_plot:
            ...
            plot_hist_dataset(adapt_ds, 'adapt_val', 'Value', 'Probability', 'Distribution used for adaptability',
                              plot_file_name_adapt)
    ...
    for dataset in datasets:
        ...
        file_ext = '.svg'
        file_name_suffix = '_avg_errors_pr'
        if display_ci:
