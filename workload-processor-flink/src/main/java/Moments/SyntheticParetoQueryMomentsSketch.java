...
public class SyntheticParetoQueryMomentsSketch implements Runnable {
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(SyntheticParetoQueryMomentsSketch.class);


    public SyntheticParetoQueryMomentsSketch(
    ...
    private void addQuery(StreamExecutionEnvironment env) {
        ...
        messageStream
            ...
            .map(new SyntheticParetoQueryMomentsSketch.DeserializeFromkafka())
            .name("DeserializeInput ")
            .disableChaining()
            .name("project ")
            .windowAll(TumblingEventTimeWindows.of(Time.seconds(windowSize)))
            .aggregate(new SyntheticParetoQueryMomentsSketch.WindowAdsAggregatorMSketch())
            .name("DeserializeInput ")
            .name("Window")
            .writeAsText("results-synp-moments.txt", FileSystem.WriteMode.OVERWRITE);
    }
    ...
    private class WindowAdsAggregatorMSketch implements AggregateFunction<
        ...
        public Tuple2<Long, SimpleMomentSketch> createAccumulator() {
            SimpleMomentSketch msketch = new SimpleMomentSketch(18);
