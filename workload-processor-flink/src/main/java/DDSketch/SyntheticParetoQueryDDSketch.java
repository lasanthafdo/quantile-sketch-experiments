...
public class SyntheticParetoQueryDDSketch implements Runnable {
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(SyntheticParetoQueryDDSketch.class);


    public SyntheticParetoQueryDDSketch(
    ...
    private void addQuery(StreamExecutionEnvironment env) {
        ...
        messageStream
            ...
            .map(new SyntheticParetoQueryDDSketch.DeserializeFromkafka())
            .name("DeserializeInput ")
            .disableChaining()
            .name("project ")
            .windowAll(TumblingEventTimeWindows.of(Time.seconds(windowSize)))
            .aggregate(new SyntheticParetoQueryDDSketch.WindowAdsAggregatorMSketch())
            .name("DeserializeInput ")
            .name("Window")
