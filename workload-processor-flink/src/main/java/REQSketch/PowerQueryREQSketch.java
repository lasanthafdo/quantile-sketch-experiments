...
public class PowerQueryREQSketch implements Runnable {
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(PowerQueryREQSketch.class);


    public PowerQueryREQSketch(
    ...
    private void addQuery(StreamExecutionEnvironment env) {
        ...
        messageStream
            ...
            .name("DeserializeInput ")
            .name("Window")
            .writeAsText("results-power-req.txt", FileSystem.WriteMode.OVERWRITE);
    }
    ...
    private static class WindowAdsAggregatorMSketch implements AggregateFunction<
        ...
        public Tuple2<Long, ReqSketch> createAccumulator() {
            ReqSketch sketch = ReqSketch.builder().setK(16).setHighRankAccuracy(false).setLessThanOrEqual(true).build();
        ...
        @Override
        public Tuple2<Long, ReqSketch> merge(Tuple2<Long, ReqSketch> acc0,
                                             Tuple2<Long, ReqSketch> acc1) {
