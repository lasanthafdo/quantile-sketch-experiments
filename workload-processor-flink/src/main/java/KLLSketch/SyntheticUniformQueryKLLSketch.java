...
public class SyntheticUniformQueryKLLSketch implements Runnable {
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(SyntheticUniformQueryKLLSketch.class);


    public SyntheticUniformQueryKLLSketch(
    ...
    private void addQuery(StreamExecutionEnvironment env) {
        ...
        messageStream
            ...
            .map(new SyntheticUniformQueryKLLSketch.DeserializeFromkafka())
            .name("DeserializeInput ")
            .disableChaining()
            .name("project ")
            .windowAll(TumblingEventTimeWindows.of(Time.seconds(windowSize)))
            .aggregate(new SyntheticUniformQueryKLLSketch.WindowAdsAggregatorMSketch())
            .name("DeserializeInput ")
            .name("Window")
            .writeAsText("results-synu-kll.txt", FileSystem.WriteMode.OVERWRITE);
    }
